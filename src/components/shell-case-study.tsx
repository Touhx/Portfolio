...
export default function ShellCaseStudy({ project }: ShellProps) {
  // Array of image slices for your case study layout.
  // Add other Shell images if available later.
  const caseStudySlices: MediaSlice[] = [
    { type: "image", src: p("/images/shell/Shell 1.png") },
    { type: "image", src: p("/images/shell/Shell 2.png") },
    { type: "image", src: p("/images/shell/Shell 3.png") },
  ];
  ...
  return (
    ...
    >
      ...
      <section className="py-32 md:py-48 px-6 text-center bg-[#050505] border-t border-white/5">
        ...
        <Link href={`/project/${project.nextProject?.slug || 'essops'}`} className="group inline-block">
          <h2 className="text-5xl md:text-[8rem] font-black uppercase tracking-tighter text-white leading-none transition-colors duration-500 group-hover:text-[#DD1D21]">
